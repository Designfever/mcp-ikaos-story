...
function buildServer() {
  ...
  server.registerTool(
    "list_stories",
    {
      ...
      inputSchema: z.object({
        status: z.enum(["ready", "waiting"]).optional(),
      }),
    },
    async ({ status }) => {
      try {
  ...
  server.registerTool(
    "get_story_rules",
    {
      ...
      inputSchema: z.object({
        rule: z.enum(
          Object.keys(STORY_RULES) as [
            keyof typeof STORY_RULES,
            ...(keyof typeof STORY_RULES)[],
          ],
        ),
      }),
  ...
  server.registerTool(
    ...
    {
      description:
        "Start or refresh Story production after the user confirms its type and compatible template. The API locks the authoritative DOCX checksum and refuses completed Stories.",
      inputSchema: z.object({
        story_id: z.string().min(1),
  ...
  server.registerTool(
    ...
    {
      description:
        "Preview or confirm a status-only update that moves every typed Story to awaiting_review. Content, images, type, template, source receipt, and revision are never included in the write.",
      inputSchema: z.object({
    ...
    async ({ mode, expected_story_ids }) => {
      try {
        if (mode === "preview")
          return output(await previewTypedStoryStatusUpdate());
        if (!expected_story_ids) {
          throw new Error(
            "expected_story_ids from preview are required for confirm",
          );
        }
  ...
  server.registerTool(
    ...
    {
      description:
        "Preview or confirm Google Sheet catalog synchronization by immutable storyKey. Only story_catalog changes; story_productions is preserved. Confirm archives the previous catalog and returns rollbackId.",
      inputSchema: z.object({
        mode: z.enum(["preview", "confirm"]),
    ...
    async ({ mode, preview_token }) => {
      try {
        ...
        if (!preview_token)
          throw new Error("preview_token from preview is required for confirm");
  ...
  server.registerTool(
    ...
    {
      description:
        "Restore the previous Story catalog snapshot for one rollbackId.",
      inputSchema: z.object({ rollback_id: z.string().uuid() }),
    },
    async ({ rollback_id }) => {
      try {
        return output(await rollbackStoryIdentity(rollback_id));
      } catch (error) {
        return failure(error);
      }
    },
  ...
  server.registerTool(
    ...
    async ({ story_id, mode, preview_token, confirmation }) => {
      try {
        if (mode === "preview")
          return output(await previewStoryReset(story_id));
        if (!preview_token)
          throw new Error("preview_token is required for confirm");
        return output(
  ...
  server.registerTool(
    ...
    {
      description:
        "Create or modify Story content/layout JSON with optimistic revision protection. Omitted read-only fields are preserved by the API; supplied read-only values cannot be changed.",
      inputSchema: z.object({
        story_id: z.string().min(1),
